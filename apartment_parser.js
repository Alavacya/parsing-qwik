...
const parseApartment = (html) => {
    ...
    const areaText = $('ul.detail-header__info-list li:contains("Общая площадь") span').text().trim();
    const area = parseArea(areaText);

    const objectType = $('ul.detail-header__info-list li:contains("Вид объекта") span').text().trim();
